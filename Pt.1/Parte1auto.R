...
results <- list(mse = c(), coef = c(c()),name = c())
...
results$mse <- c(results$mse, lm.mse)
results$coef <- c(results$coef, "lm.coef")
results$name <- c(results$name, "Linear Model")
...
results$mse <- c(results$mse, bwd.mse)
results$coef <- c(results$coef, "bwd.coef")
results$name <- c(results$name, "Backward Stepwise")
...
results$mse <- c(results$mse, fwd.mse)
results$coef <- c(results$coef, "fwd.coef")
results$name <- c(results$name, "Forward Stepwise")
...
results$mse <- c(results$mse, hyb.mse)
results$coef <- c(results$coef, "hyb.coef")
results$name <- c(results$name, "Hybrid Stepwise")
...
results$mse <- c(results$mse, ridge.mse)
results$coef <- c(results$coef, "ridge.coef")
results$name <- c(results$name, "Ridge")
...
results$mse <- c(results$mse, lasso.mse)
results$coef <- c(results$coef, "lasso.coef")
results$name <- c(results$name, "Lasso")
...
results$mse <- c(results$mse, enet.mse)
results$coef <- c(results$coef, "enet.coef")
results$name <- c(results$name, "ElasticNet")

####################################################
################# Final Analysis ###################
####################################################

results.mse <- min(results$mse)
index <- which(results$mse == results.mse)
results.name <- results$name[index]
results.coef <- get(results$coef[index])
print(paste("The model with minimum MSE is:",results.name))
print(paste("The MSE is:",results.mse))
print("The significant regressors are:");results.coef
predictors <- round(results.coef / 100, digits = 0) # Round the values in bwd.coef and divide by 100, with 0 decimal places
print(paste("The word is:",intToUtf8(predictors))) # Convert the values in bwd.predictors to UTF-8 encoded integers