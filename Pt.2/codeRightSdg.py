...
def sgd(m, x_train, y_train, lr=0.0001):
    '''
...
def try_lr(lr_array):
    ...
    '''
    x = [i for i in range(1, X_train_scale.shape[0]+1)]
    plt.figure("Gradient descent")
    
    for lr_i in lr_array:
        print("I'm trying with lr=", lr_i)
        Beta, loss = sgd(m, X_train_pca_scale_reduced,
                         y_train, lr=lr_i)

        plt.plot(x,loss, label=lr_i)
...
try_lr([0.0001, 0.005, 0.001, 0.01, 0.1])

Beta, loss = sgd(m, X_train_pca_scale_reduced,
                 y_train, lr=lr)

x = [i for i in range(1, X_train_scale.shape[0]+1)]
plt.figure("Gradient descent, lr=" + str(lr))
plt.plot(x,loss)
plt.xlabel('Observation')
plt.ylabel('Loss')
plt.title("Loss history, lr=" + str(lr))
